...
// CRM: todas as reservas (com filtros opcionais)
// Público: apenas as próprias
...
// ── POST /api/reservations — criar (usuário público) ────────────
router.post('/', auth, async (req, res) => {
  if (req.user.type !== 'public')
    return res.status(403).json({ error: 'Somente usuários públicos podem reservar' });

  const { point_id, est_id, date, start_time, end_time, hours } = req.body;
  ...
  try {
    ...
    const { rows } = await pool.query(`
      ...
      [point_id, est_id, req.user.id, date, start_time, end_time, hours, total]
    );

    // Busca dados completos para email
    const { rows: full } = await pool.query(
      `${RES_QUERY} WHERE r.id = $1`, [rows[0].id]
    );
...
router.patch('/:id/cancel', anyAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `${RES_QUERY} WHERE r.id = $1`, [req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Reserva não encontrada' });
    const res_ = rows[0];

    // Usuário público só pode cancelar a própria reserva com 2h de antecedência
    if (req.user.type === 'public') {
    ...
    await pool.query(
      "UPDATE reservations SET status='cancelled' WHERE id=$1", [req.params.id]
    );
...
router.patch('/:id/reschedule', anyAuth, async (req, res) => {
  ...
  try {
    ...
    if (req.user.type === 'public') {
      ...
      const cutoff = new Date(dt.getTime() - 2 * 60 * 60 * 1000);
      if (new Date() >= cutoff)
        return res.status(400).json({ error: 'Prazo de remarcação encerrado (2h antes)' });
    }

    // Verifica conflito no novo horário (excluindo a própria reserva)
    const { rows: conflicts } = await pool.query(`
      SELECT id FROM reservations
    ...
    if (conflicts.length)
      return res.status(409).json({ error: 'Horário indisponível no novo horário' });

    const total = res_.price_per_hour * hours;
    const { rows: updated } = await pool.query(`
      UPDATE reservations
      SET date=$1, start_time=$2, end_time=$3, hours=$4, total=$5
