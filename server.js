...
async function runMigrations() {
  const stmts = [
    ...
    `ALTER TABLE professores       ADD COLUMN IF NOT EXISTS valor_hora_avulso  NUMERIC(10,2) DEFAULT 0`,
    `ALTER TABLE professores       ADD COLUMN IF NOT EXISTS percentual_repasse NUMERIC(5,2)  DEFAULT 0`,
    ...
    // Permite reservas manuais sem ponto/data/horário (clientes que não usam o calendário)
    `ALTER TABLE reservations ALTER COLUMN point_id    DROP NOT NULL`,
    `ALTER TABLE reservations ALTER COLUMN date        DROP NOT NULL`,
    ...
    `CREATE TABLE IF NOT EXISTS aulas_avulsas (
      ...
      ponto_id      INTEGER REFERENCES points(id)        ON DELETE SET NULL,
